...
if(!empty($_POST)){
    ...
    $pic3 = ( empty($pic2) && !empty($dbFormData['pic3'])) ? $dbFormData['pic3'] : $pic3;
    
    //更新の場合はDBの情報と入力情報が異なる場合にバリデーション
    if(empty($dbFormData)){
        //未入力チェック
    ...
    if(empty($err_msg)){
        ...
        try {
            $dbh = dbConnect();
            // 編集画面の場合はUPDATE、新規登録画面の場合はINSERT文を生成
            if($edit_flg){
                debug('DB更新です。編集');
                ...
                $data = array(':name' => $name, ':category' => $category, ':price' => $price, ':comment' => $comment, ':pic1' => $pic1, ':pic2' => $pic2, ':pic3' => $pic3);
            }else{
                debug('DB新規登録です');
        ...
        <section id="main">
            <div class="form-container">
                <form action="" method="post" class="form" enctype="multipart/form-data" style="width:100%;box-sizing:border-box;">
                    ...
                    <p class="count-text"><span id="js-count-view">0</span>/500文字</p>
                    <div class="area-msg">
                        <?php if(!empty($err_msg['comment'])) echo $err_msg['comment']; ?>
                    </div>
                    <label for="" class="<?php if(!empty($err_msg['price'])) echo 'err'; ?>">
                        金額<span class="label-require">必須</span>
                        <input type="text" name="price" style="width:150px;" placeholder="0" value="<?php echo (!empty(getFormData('price'))) ? getFormData('price') : 0; ?>"><span class="option">円</span>
                    </label>
                    <div class="area-msg">
                        <?php
